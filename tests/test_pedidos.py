...
@pytest.mark.smoke
def test_criar_pedido_com_prato_disponivel(client):
...
def test_criar_pedido_com_prato_indisponivel_retorna_400(client):
    client.put("/pratos/2/disponibilidade", json={"disponivel": False})
    payload = {"prato_id": 2, "quantidade": 1}
    response = client.post("/pedidos", json=payload)
    assert response.status_code == 400
    # restaura
    client.put("/pratos/2/disponibilidade", json={"disponivel": True})
