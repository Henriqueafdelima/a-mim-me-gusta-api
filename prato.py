...
class PratoInput(BaseModel):
    ...
    categoria: str = Field(
        pattern="^(burrito|taco|quesadilla|sobremesa|entrada|salada)$",
        description="Categoria do prato"
    )
    preco: float = Field(gt=0, description="Preço em reais, deve ser positivo")
    preco_promocional: Optional[float] = Field(default=None, gt=0, description="Preço promocional opcional")
    descricao: Optional[str] = Field(default=None, max_length=500)
    disponivel: bool = True

    # Exercício 2.4 — validador customizado
